package com.rui.n.ES.elasticsearch.test;
...
import com.rui.n.ES.elasticsearch.utils.Client;

public class TestentityForES {

	public static final String DEFAULT_REGEX = "^ java.|^   at";
	
	public String format(String log,Client client) {
		...
		if (facility_matcher.find()) {
				...
				if(m.find()) {
					...
					if (builder.length()!=0) {
						...
						try {
							...
							System.out.println(logType+"范式化报错！"+builder.toString());
						}
			...
			try {
				...
				System.out.println(logType+"范式化报错！"+log);
			}
		...
		//es暂无防火墙包过滤日志对应的mapping，暂未入库es
			...
			if (syslog.getIp().equals("10.60.248.132")) {
				syslog.setUserid("e3e8b0b22455458d861cdec657b5ff24");
				syslog.setDeptid(String.valueOf(177));
				syslog.setEquipmentid("da6a9627d47f498bb4bea6694022bb09");
				syslog.setEquipmentname("报社测试");
	...
	public static void main(String[] args) {
		...
		/*if (args.length<2) {
			...
			TestentityForES testentity = new TestentityForES();
		...
		File file = new File("C:\\Users\\jiyourui\\Desktop\\test3.log");
		try{
            BufferedReader br = new BufferedReader(new FileReader(file));//构造一个BufferedReader类来读取文件
            String s = null;
            while((s = br.readLine())!=null){//使用readLine方法，一次读一行
                //result.append(System.lineSeparator()+s);
            	testentity.format(s,client);
            }
            br.close();    
        }catch(Exception e){
            e.printStackTrace();
